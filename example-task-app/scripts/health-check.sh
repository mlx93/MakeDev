...
# Usage: health-check.sh [timeout_seconds] (default: 300)

set -e

TIMEOUT=${1:-300}  # Default 5 minutes, can be overridden
INTERVAL=5   # Check every 5 seconds
ELAPSED=0

FRONTEND_URL="http://localhost:3000/health"
BACKEND_URL="http://localhost:8080/api/v1/health"
BACKEND_READY_URL="http://localhost:8080/api/v1/health/ready"
...
while [ $ELAPSED -lt $TIMEOUT ]; do
    ...
    # Display status with clear indicators
    STATUS=""
    [ "$BACKEND_OK" = true ] && STATUS="${STATUS}✅ backend " || STATUS="${STATUS}❌ backend "
    [ "$BACKEND_READY_OK" = true ] && STATUS="${STATUS}✅ backend-ready " || STATUS="${STATUS}❌ backend-ready "
    [ "$FRONTEND_OK" = true ] && STATUS="${STATUS}✅ frontend " || STATUS="${STATUS}❌ frontend "
