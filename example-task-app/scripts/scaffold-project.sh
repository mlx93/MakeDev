...
# Check if directories already exist - if so, skip scaffolding
if [ -d "$FRONTEND_PATH" ] || [ -d "$BACKEND_PATH" ]; then
    echo "✅ frontend/ or backend/ directories already exist"
    echo "   Skipping scaffolding - using existing project structure"
    exit 0
fi
...
if [ ! -d ".git" ]; then
    ...
    git commit -m "Initial scaffold from zero-to-running-dev-env" --quiet > /dev/null 2>&1 || true
